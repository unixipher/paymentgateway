...
import { notifyFailedOrders, openCheckout, orderState, publicOrderView } from '@/lib/orders';
...
/**
 * Checkout status for the payer's page, which polls this every few seconds. The first call starts the
 * payment timer. While the order is open,
...
export const GET = handler(async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  ...
  // The first time the payer opens the link, their payment timer starts.
  const order = await openCheckout(found);
